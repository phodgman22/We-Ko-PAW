...
5. **Unscoped localStorage/sessionStorage keys leaked between separate event copies of this
   ...
   app opened in that same browser: discovered right here, when a leftover draft from testing
   the template showed up in this repo's console the first time it was opened. Fixed by
   namespacing every one of these keys with the Firebase project id
   ...
   namespacing it the same way. Fixed in the template too, so it won't recur on the next fork.
